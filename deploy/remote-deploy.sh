...
: "${DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD_B64:?DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD_B64 is missing}"
: "${DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN_B64:?DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN_B64 is missing}"
...
: "${LISTENBRAINZ_TOKEN_B64:=}"
: "${LISTENBRAINZ_API_BASE_URL_B64:?LISTENBRAINZ_API_BASE_URL_B64 is missing}"
: "${DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT_B64:?DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT_B64 is missing}"
: "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE_B64:?DISCORD_BOT_LISTENBRAINZ_RADIO_MODE_B64 is missing}"
...
DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD="$(decode "${DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD_B64}")"
DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN="$(decode "${DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN_B64}")"
...
LISTENBRAINZ_TOKEN="$(decode "${LISTENBRAINZ_TOKEN_B64}")"
LISTENBRAINZ_API_BASE_URL="$(decode "${LISTENBRAINZ_API_BASE_URL_B64}")"
DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT="$(decode "${DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT_B64}")"
DISCORD_BOT_LISTENBRAINZ_RADIO_MODE="$(decode "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE_B64}")"
...
if [[ ! "${DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD}" =~ ^[0-9]+$ ]] \
    || (( DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD < 1 \
      || DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD > 10 )); then
  echo "Playback provider failure threshold must be between 1 and 10" >&2
  exit 1
fi
if [[ ! "${DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN}" =~ ^[1-9][0-9]*(ms|s|m)$ ]]; then
  echo "Playback provider cooldown must be a positive duration such as 90s" >&2
  exit 1
fi
...
if [[ ! "${LISTENBRAINZ_API_BASE_URL}" =~ ^https:// ]]; then
  echo "ListenBrainz API base URL must use https" >&2
  exit 1
fi
if [[ ! "${DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT}" =~ ^[0-9]+$ ]] \
    || (( DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT < 3 || DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT > 50 )); then
  echo "Collaborative artist limit must be between 3 and 50" >&2
  exit 1
fi
if [[ "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE}" != "easy" \
    && "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE}" != "medium" \
    && "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE}" != "hard" ]]; then
  echo "ListenBrainz radio mode must be easy, medium or hard" >&2
  exit 1
fi
...
write_env() {
  ...
  temp_file="$(mktemp "${DEPLOY_DIR}/.env.XXXXXX")"
  {
    ...
    printf 'DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD=%s\n' "${DISCORD_BOT_PLAYBACK_PROVIDER_FAILURE_THRESHOLD}"
    printf 'DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN=%s\n' "${DISCORD_BOT_PLAYBACK_PROVIDER_COOLDOWN}"
    ...
    printf 'LISTENBRAINZ_TOKEN=%s\n' "${LISTENBRAINZ_TOKEN}"
    printf 'LISTENBRAINZ_API_BASE_URL=%s\n' "${LISTENBRAINZ_API_BASE_URL}"
    printf 'DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT=%s\n' "${DISCORD_BOT_COLLABORATIVE_ARTIST_LIMIT}"
    printf 'DISCORD_BOT_LISTENBRAINZ_RADIO_MODE=%s\n' "${DISCORD_BOT_LISTENBRAINZ_RADIO_MODE}"
  } > "${temp_file}"
  chmod 600 "${temp_file}"
